...
import { ArrowRightLeft, Check, X, Clock, MessageCircle } from 'lucide-react';
...
const TradeRequestCard = ({ trade, isIncoming, onRefresh }) => {
  ...
  const handleAction = async (action) => {
    try {
      if (action === 'accept') {
        await api.put(`/trades/${trade._id}/accept`);
        toast.success("Trade Accepted! Escrow locked.");
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex gap-3">
        {trade.status === 'pending' && isIncoming && (
          <>
            ...
            <button 
              ...
              className="flex-1 py-2 bg-white/60 text-red-700 rounded-xl font-bold text-sm border border-red-100 hover:bg-red-50 transition-colors flex items-center justify-center gap-2"
            >
              <X size={16} /> Reject
            </button>
          </>
        ...
        {/* If trade is Active (Accepted), Show Chat Button */}
        {trade.status === 'accepted' && (
           <button 
             onClick={() => navigate(`/trades/${trade._id}`)}
             className="w-full py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-md hover:bg-indigo-500 transition-colors flex items-center justify-center gap-2"
           >
             <MessageCircle size={16} /> Enter Trade Room
           </button>
        )}
        
        {trade.status === 'pending' && !isIncoming && (
           <div className="w-full py-2 text-center text-xs font-medium text-emerald-900/50 flex items-center justify-center gap-1">
